...
# ╔═╡ 3a3fccfe-be2d-11f0-889d-1de29f233680
using CUDA

# ╔═╡ 4b84caa1-3160-40ed-876d-2e03a7fbf19f
using cuDNN

# ╔═╡ 4eb04966-f528-4873-b43b-38839573f3b9
using ObjectDetector # Esta es la biblioteca que implementa YOLO

# ╔═╡ f037bfc6-5a2c-46b9-a6fc-2e8b774179cf
using FileIO

# ╔═╡ 904f3ab3-18fd-4536-8138-d6db3f5ac6d9
using ImageIO

# ╔═╡ 73bea108-1f3b-484a-95d7-71e9151cde83
using ImageView

# ╔═╡ 7e0a06ac-7dd8-48a7-bac4-d72a53c270db
using ObjectDetector.YOLO

# ╔═╡ d9dc7d37-328a-4f1a-aac9-b7b4691517e8
using Images

# ╔═╡ e5c227ce-c427-4035-8a06-66adb42873e4
using Cairo

# ╔═╡ 76d02473-9d87-487f-ad76-088c403623da
using HTTP

# ╔═╡ 65bbcdab-ea70-4782-bebd-f8304e46d37a
md"""
Cargamos YOLO versión 7.
"""

# ╔═╡ 3b8c8ffd-7cae-46ae-9622-5cc9723f8819
function prepara_yolo()
    yolomod = YOLO.v7_416_COCO(batch=1, silent=true) # Carga YOLO
    batch = emptybatch(yolomod) # Crea un lote vacia
    return yolomod, batch
end

# ╔═╡ 6885b6c3-4f46-4423-8f5c-5db70b65ea20
md"""
Le pedimos a YOLO que prepare la imagen para procesarla.
"""
...
# ╔═╡ d01da292-9c6e-4bb9-8cd6-ee17f933bff8
md"""
Lanzamos la detección y ubicación de los objetos en la imagen.
"""

# ╔═╡ ee5946fa-1fbb-4e44-b5f3-d84179e2b277
md"""
Nos devuelve la configuración de YOLO
"""

# ╔═╡ 58ef139d-703e-41e5-ba83-f602ed12f7f5
get_cfg(y::YOLO.Yolo) = y.cfg

# ╔═╡ cb617a50-c360-49f4-a8b0-be576d7c7395
md"""
Genera colores aleatorios para los recuadros de los objetos.
"""

# ╔═╡ ca6ae1bb-3921-4466-a6ff-fa918c7da23d
function genera_colores_clases(model::YOLO.Yolo)
    clases = get_cfg(model)[:output][1][:classes]
    semillas = [RGB{N0f8}(0,0,0), RGB{N0f8}(1,1,1)]
    return Colors.distinguishable_colors(clases, semillas; dropseed=true)
end

# ╔═╡ a11dc234-a2ab-433c-945c-cca1ddeee8c2
md"""
Dibuja los recuadros, y las etiquetas de las clases junto con la confianza de la clasificación.
"""

# ╔═╡ 2e6c2132-e573-4e19-9c38-722ae3a80fb4
function dibuja_recuadros!(img::Union{Matrix{RGBA{N0f8}},Matrix{RGB{N0f8}}}, model::YOLO.Yolo, padding::AbstractArray, results, names;
    transpose=true, fontsize=48, opacity=0.8, colores = nothing, kwargs...)

    if colores === nothing
        colores = genera_colores_clases(model)
    end
    ...
    for i in axes(results,2)
        # Extrae y escala el recuadro envolvente
        ...
        Cairo.rectangle(ctx, x1, y1, x2 - x1, y2 - y1)
        Cairo.set_source_rgba(ctx, red(color), green(color), blue(color), opacity)
        Cairo.stroke(ctx)

        # prepara la etiqueta
        class_id = Int(results[end-1, i])
        name = names[class_id]
        label = name * " $(round(Int, conf*100))"
        tx, ty = x1 + 1, y1 - 4.5  # 4px arriba izquierda

        # dibuja el texto de la etiqueta
        Cairo.set_font_face(ctx, "sans-serif $(fontsize)px")
		Cairo.set_font_size(ctx, fontsize)
        Cairo.move_to(ctx, tx, ty)
        Cairo.set_source_rgba(ctx, 0, 0, 0, opacity)
        x_bearing, y_bearing, width, height, x_advance, y_advance = Cairo.text_extents(ctx, label)
        pad = 2.0
        bx = tx + x_bearing - pad
        by = ty + y_bearing - pad
        bw = width + 2*pad
        bh = height + 2*pad
        Cairo.save(ctx)
        Cairo.set_source_rgba(ctx, 1, 1, 1, opacity)
        Cairo.rectangle(ctx, bx, by, bw, bh)
        Cairo.fill(ctx)
        Cairo.restore(ctx)
        Cairo.set_source_rgba(ctx, 0, 0, 0, opacity)
        Cairo.move_to(ctx, tx, ty)
        Cairo.show_text(ctx, label)
    end
...
# ╔═╡ 9b676e14-57ed-4563-aa50-f06bc2af4b82
md"""
Las dos funciones siguientes son las que reparten el juego.
"""

# ╔═╡ 02c7e590-63e8-4385-a4f0-cda6b1ed1fdb
function main_detecta(path::String, yolomod, batch, names)
    img, batch, padding = prepara_imagen(path, yolomod, batch)
    return detecta(img, yolomod, padding, batch, names)
end

# ╔═╡ 556e602f-07c4-4f55-9258-66f91d1c4265
function main_prepara()
    yolomod, batch = prepara_yolo();
    names = collect(eachline(joinpath(ObjectDetector.YOLO.models_dir(), "coco.names")))

    return yolomod, batch, names
end

# ╔═╡ de322ad7-7c4f-4496-9cd5-05690890be2c
md"""
Indicamos la ruta a la imagen.
"""

# ╔═╡ 2cf4ee32-be47-48eb-8986-3f9799f7bf31
path = "https://belmonte.uji.es/Docencia/IR2130/Teoria/RedesNeuronales/Imagenes/columpio.jpg"

# ╔═╡ 127f024b-966a-4774-837c-f3f0d7fc75a0
yolomod, batch, names = main_prepara()

# ╔═╡ ff4b28f1-b1f1-42b1-9c35-0c5ab11a4c98
md"""
Hacemos la detección
"""

# ╔═╡ 579264e9-ec93-4e95-b30d-6464b2da1023
main_detecta(path, yolomod, batch, names)
...
manifest_format = "2.0"
project_hash = "77564a4f03654954f92134005583b4e10f7e12e0"
...
[[deps.AllocArrays]]
deps = ["Bumper", "ConcurrentUtilities", "PrecompileTools", "ScopedValues"]
git-tree-sha1 = "9080af01be1fc55e440fe817420233d249f38cd2"
uuid = "5c00bae2-1499-4716-9206-27f63fd08a44"
version = "1.1.0"
weakdeps = ["Adapt"]

    [deps.AllocArrays.extensions]
    AdaptExt = "Adapt"
...
[[deps.Bumper]]
deps = ["UnsafeArrays"]
git-tree-sha1 = "e3cd36a329dc00c8ced78cc0e89edada02763f40"
...
git-tree-sha1 = "fd3cf47e4be66fc957996fdb5ba41283d0adaf85"
...
git-tree-sha1 = "9f740477f24feff51a6fa4b18d71c939daa29cda"
...
[[deps.ObjectDetector]]
deps = ["Adapt", "AllocArrays", "BenchmarkTools", "Cairo", "Colors", "Flux", "ImageCore", "ImageDraw", "ImageFiltering", "ImageTransformations", "LazyArtifacts", "PrecompileTools", "PrettyTables", "TimerOutputs", "UnsafeArrays"]
git-tree-sha1 = "ac2434dbd21031226038ca15d4f264a0a6b97972"
...
git-tree-sha1 = "b832b4e26effbb8ef9877811ab63f838b4c9c3e0"
