...
hubble_url = "https://raw.githubusercontent.com/AprendizajeAutomaticoUJI/DataSets/refs/heads/master/hubble.csv"

# ╔═╡ c9195f98-ad36-4264-80e8-4c2f7078213a
hubble = CSV.File(HTTP.get(hubble_url).body) |> DataFrame

# ╔═╡ ac05537e-80da-4d04-b5a3-557e88ad59a8
md"""
Estadística descriptiva básica del conjunto de datos:
"""
...
md"""
Visualizamos la magnitud aparente (relacionada con la distancia), frente a la velocidad radial (calculada a partir del desplazamiento al rojo):
"""

# ╔═╡ fdb31fef-9990-47aa-90d9-73093cbca53e
scatter(hubble.mean_m, hubble.velocity,
	title = "Velocidad radial vs. magnitud",
	xlabel = "Magnitud aparente",
	ylabel = "Velocidad radial (km./s.)"
)

# ╔═╡ b91a3089-6669-4f12-8b95-b763435f29fb
md"""
Ecuación que relaciona la magnitud aparente de un objeto con su distancia a la Tierra:
"""

# ╔═╡ 28c9bf35-0593-45a6-b819-a889a22e2ac8
function distancia(magnitud::Float64)::Float64
	M = -13.8
	d = exp10((magnitud-M+5.0)/5.0)/1000_000
	return d # distancia en Mparsecs
end

# ╔═╡ f38b7c83-1a50-4525-af72-d7c8625eb0c9
function distancia_corregida(magnitud::Float64)::Float64
	M = -13.8
	d = exp10((magnitud-M+5)/5.0)/1000_000
	return 7.3*d # distancia en Mparsecs
end

# ╔═╡ 65b5ed4a-cbbb-4659-b373-62a76acd2370
md"""
Visualizamos la distancia frente a la velocidad radial (Ley de Hubble)

```math
v = H_0 d
```

Las unidades de ``H_0`` son (Km/s)/Mparsec, y la distancia ``d`` se mide en [parsecs](https://es.wikipedia.org/wiki/P%C3%A1rsec).
"""

# ╔═╡ a2ba84e6-bb49-4720-b84b-12796df7b37e
scatter(distancia.(hubble.mean_m), hubble.velocity,
	title = "Ley de Hubble",
	xlabel = "Distancia a la Tierra (parsecs)",
	ylabel = "Velocidad radial (km./s.)"
)

# ╔═╡ 091fb07a-c52b-4c45-a1a0-9220bf5ff3bb
md"""
Vamos a hacer la regresión lineal de los datos:
"""

# ╔═╡ d5c59a23-2d04-43ee-9b7e-9d976449fe96
RegresorLineal = @load LinearRegressor pkg = MLJLinearModels verbosity = 0
...
y = hubble_coercion.velocity

# ╔═╡ 92dabed3-eb23-451e-aab8-78e1e4276fd1
maquina = machine(RegresorLineal(), X, y) |> fit!

# ╔═╡ be4ada1e-e39c-4471-8b09-3fb955dfbabe
H0 = fitted_params(maquina)[1][1][2]
...
begin
	...
	plot!(distancia.(hubble.mean_m), predict(maquina, X),
		 width = 2
	)
end
...
md"""
Sencillo (y casualmente preciso) cálculo de la edad del universo a partir de la constante de Hubble.

Según la ley de Hubble ``v = H_0 d`` por otro lado, la definición de velocidad es ``v = \frac{d}{t}``, igualando ambas expresiones tenemos que:

```math
t = \frac{1}{H_0}
```

donde ``t`` se corresponde con la edad del universo.

Como la distancias están medidas en parsecs, y 1 parsec son 3.2616 años luz:

t = $(3.2616 * 300_000 / H0) millones de años.
...
git-tree-sha1 = "b21d44238071011d53da10557a922cdbe074aebb"
...
git-tree-sha1 = "44e5ecbe634d6dcb6a32328eecd83ea97853eb59"
...
git-tree-sha1 = "0638d603308da3eb6bd16fe2384ecb0b91b8b9c2"
