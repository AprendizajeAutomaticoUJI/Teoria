...
# ╔═╡ 43890210-3444-4588-a155-19cc2c74a2b9
TableOfContents(title = "Contenidos", depth = 1)

# ╔═╡ 86f59322-8acd-4619-8367-2458849c1b05
md"""
...
# ╔═╡ b083fc2d-3320-449b-b40a-5405d0c5deed
md"""
# Cálculo de la actualización

La siguiente función calcula la actualización que se le resta a la x actual. Para calcular la derivada de la función en el punto estamos utilizando el paquete **Zygote**:
"""
...
# ╔═╡ 0197f60d-ee9a-418d-b855-9e6ee872eab1
md"""
La siguiente función calcula todas las x por las que pasa el descenso de gradiente cuando se define un número determinado de pasos.
"""

# ╔═╡ e933a965-33a8-4db5-803c-f419377737f5
md"""
# Descenso de gradiente
"""

# ╔═╡ 434e010a-ebc0-42a1-81ac-21ae87108d2e
function descenso(f, x₀, η, pasos)
	xₜ = x₀
	resultado = [xₜ]
	for i in 1:pasos
		xnew = xₜ - η * actualizacion(xₜ, f, η)
		push!(resultado, xnew)
		xₜ = xnew
	end
	return resultado
...
md"""
La siguiente función calcula todas las x por las que pasa el descenso de gradiente cuando se establece un límite entre dos actualizaciones sucesivas, si la diferencia entre el nuevo valor de $x_t$ y el nuevo $x_{y+1}$ está por debajo de un umbral $\epsilon$, hemos llegado cerca del mínimo.
"""

# ╔═╡ 2e1c7807-e953-4d99-979e-f4071783c51d
function descenso_con_umbral(f, x₀, η, ϵ)
	xₜ = x₀
...
md"""
Elije una función: 
$(@bind ejemplo_seleccionado Select(sort(collect(keys(opciones)))))
"""

# ╔═╡ 9891262e-6dbe-4f57-8800-ffca61698353
opcion = opciones[ejemplo_seleccionado];

# ╔═╡ 83b0924a-30aa-4dc4-ac85-93ea45e21728
function dibuja_resultado(resultado)
...
Columns(
	md""" Fija el tamaño del paso **η**:""",
	@bind η Slider(0.1:0.1:1, default = 0.5, show_value = true);
	widths = [30, 70]
)

# ╔═╡ 7d3eafd8-e3c1-4aec-a3a6-680d39f198c3
Columns(
	md""" Fija el tamaño  del umbral **ϵ**:""",
	@bind ϵ Slider(1e-2:1e-2:1e-1, default = 1e-1, show_value = true);
	widths = [30,70]
)

# ╔═╡ fd1a54a2-98b8-4872-b665-8dd78d40968a
resultado = descenso_con_umbral(opcion.funcion, opcion.x₀, η, ϵ);
