...
plot_co2 = plot(co2.CO2,
	 xticks = (1:12:size(co2,1),string.(co2.Año[1:12:size(co2,1)])),
	 xrotation = 45,
	 title = "Concentración de CO2 en Izaña, Tenerife",
	 legends = false,
	 xlabel = "Año",
...
plot_autocorrelacion_co2 = plot(autocor(co2.CO2),
	 seriestype = :scatter,
	 title = "Autocorrelación para la serie de datos CO2",
	 xlabel = "Retraso",
...
function plot_tendencia_esatacionalidad()
	...
	p1 = plot!(
		media_deslizante(co2.CO2, 12),
		label = "Tendencia",
	)
	plot()
	p2 = plot(
		co2.CO2[13:end] - media_deslizante(co2.CO2, 12),
...
Este es el despliegue de la rede recurrente a lo largo 
del tiempo.
"""
